...
awk -v anchor="$anchor" '
  $0 == anchor {
    ...
    print "    let mut store = Store::new(wasmer::wasmi::Wasmi::new());"
    print "    #[cfg(not(all(target_os = \"ios\", not(target_abi = \"sim\"))))]"
    print "    let mut store = Store::default();"
    print "    // Says which backend on the device console, where a witness is the"
    print "    // one thing whose failure mode is the process disappearing."
    print "    eprintln!("
    print "        \"railgun: witness store backend = {}\","
    print "        store.engine().deterministic_id()"
    print "    );"
    patched++
    next
  }
